...
pVertex addVertex(pGragh head,void* item) {
    ...
    if (head->count==0) {
        ...
        pVertex tpe=head->first;
        while (tpe->pnextvertex!=NULL) {
          tpe=tpe->pnextvertex;
        }
        tpe->pnextvertex=temp;
        ...
        for (int i=0;i<size;i++) {
            (head->pArc)[i]=(int*)realloc((head->pArc)[i],sizeof(int)*(size));//기존에 있는 행에 한칸 열 추가
            (head->pArc)[i][size-1]=0;// 그 추가된 칸열값을 0으로 함.
        }
        memset((head->pArc)[size-1],0,size*sizeof(int));//마지막 새로 추가된 vertex에 대한 모든 관계 전부 0으로 초기화
    }
